...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        fx_diff = torch.norm(output1 - output2, p=2, dim=1, keepdim=True)
        L = torch.mean((1-label)*fx_diff**2 + label * torch.relu(self.margin - fx_diff)**2)
        return L
...
def evaluate(dataiter, net, split, device):
    for i in range(2):
        ...
        for j in range(10):
            ...
            plt.savefig('results/task2/lfw/%s_%d_%d.png'%(split,i, j))
            plt.close()
...
if __name__ == "__main__":
    ...
    siamese_dataset = SiameseNetworkDataset(imageFolderDataset=folder_dataset,
                                        transform=transforms.Compose([transforms.Resize((100,100)),
                                                                      transforms.ToTensor()]),
                                        should_invert=False, should_gray=True)
    ...
    plt.savefig('results/task2/lfw/loss_hist.jpg')   
