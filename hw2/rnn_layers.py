...
def rnn_step_backward(dnext_h, cache):
    ...
    dx = dbefore_act.dot(Wx.T)
    dprev_h = dbefore_act.dot(Wh.T)
    dWx = x.T.dot(dbefore_act)
    dWh = prev_h.T.dot(dbefore_act)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        if i == T-1:
            ...
            dh_i = dh[i] + dprev_h_t
            dx_t, dprev_h_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh_i, cache[i])
        dWh += dWh_t
        dWx += dWx_t
        dx[i] = dx_t
        db += db_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    fico = prev_h.dot(Wh) + x.dot(Wx) + b
    f_t, i_t, C_t_wave, o_t = np.split(fico, 4, axis=1) # (N, H)

    f_t = sigmoid(f_t) # (N, H)
    i_t = sigmoid(i_t) # (N, H)
    C_t_wave = np.tanh(C_t_wave) # (N, H)
    o_t = sigmoid(o_t) # (N, H)

    next_c = f_t * prev_c + i_t * C_t_wave
    next_h = o_t * np.tanh(next_c)

    cache = (x, prev_h, prev_c, Wx, Wh, f_t, i_t, o_t, C_t_wave, next_c, next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, Wx, Wh, f_t, i_t, o_t, C_t_wave, next_c, next_h = cache

    dC_t = dnext_h * o_t * (1 - np.tanh(next_c) ** 2) + dnext_c
    df_t = dC_t * prev_c * f_t * (1 - f_t)
    di_t = dC_t * C_t_wave * i_t * (1 - i_t)
    dC_t_wave = dC_t * i_t * (1 - C_t_wave ** 2)
    dprev_c = dC_t * f_t
    do_t = dnext_h * np.tanh(next_c) * o_t * (1 - o_t)

    dfico = np.concatenate([df_t, di_t, dC_t_wave, do_t], axis=1) # (N, 4H)
    dWx = x.T.dot(dfico)
    dWh = prev_h.T.dot(dfico)

    db = np.sum(dfico, axis=0)

    dx = dfico.dot(Wx.T)
    dprev_h = dfico.dot(Wh.T)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        if i == 0:
            h[i], c[i], cache_t = lstm_step_forward(x[i], h0, c0, Wx, Wh, b)
        else:    
            h[i], c[i], cache_t = lstm_step_forward(x[i], h[i-1], c[i-1], Wx, Wh, b)
        cache.append(cache_t)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        if i == T-1:
            dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh[i], dc, cache[i])
        else:
            dh_i = dh[i] + dprev_h_t
            dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh_i, dprev_c_t, cache[i])
        dWh += dWh_t
        dWx += dWx_t
        dx[i] = dx_t
        db += db_t
...
def temporal_fc_forward(x, w, b):
    ...
    N, T, D = x.shape
    _, M = w.shape
    out = (x.reshape(N * T, D).dot(w) + b).reshape(N, T, M)
    cache = (x, w, b)
...
def temporal_fc_backward(dout, cache):
    ...
    dx = dout.reshape(N * T, M).dot(w.T).reshape(N, T, -1)
    dw = x.reshape(N * T, -1).T.dot(dout.reshape(N * T, M))
