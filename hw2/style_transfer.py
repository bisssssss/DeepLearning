...
def content_loss(content_weight, content_current, content_target):
    ...
    L_content = content_weight * torch.sum((content_current - content_target) ** 2)
    return L_content
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for l in range(len(style_layers)):
        L_style += style_weights[l] * torch.sum((gram_matrix(feats[style_layers[l]]) - style_targets[l]) ** 2)
    return L_style
...
def tv_loss(img, tv_weight):
    ...
    L_tv = torch.sum((img[:, :, :, :-1] - img[:, :, :, 1:]) ** 2)
    L_tv += torch.sum((img[:, :, :-1, :] - img[:, :, 1:, :]) ** 2)
    return tv_weight * L_tv
...
def style_transfer(content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    plt.figure()
    loss_hist = []
    content_name = content_image.split('/')[1].split('.')[0]
    ...
    for t in range(200):
        ...
        # TODO:Compute loss
        L_content = content_loss(content_weight, feats[content_layer], content_target)
        L_style = style_loss(feats, style_layers, style_targets, style_weights)
        L_tv = tv_loss(img_var, tv_weight)
        loss = L_content + L_style + L_tv
        loss.backward()
    ...
    plt.axis('off')
    plt.imshow(deprocess(img.cpu().clone()))
    plt.savefig(content_name+'_'+style_name+'.png')
    plt.show()
    plt.close()

    # save learning curve of loss
    f = plt.figure(figsize=(8,4))
    plt.semilogy(loss_hist)
    plt.xlabel('Iteration')
    plt.ylabel('loss')
    plt.title('learning curve of loss, content: '+content_name+', style: '+style_name)
    plt.grid()
    plt.savefig('loss_'+content_name+'_'+style_name+'.png')
    plt.close()
