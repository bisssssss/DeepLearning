...
def discriminator_loss(logits_real, logits_fake, dtype):
    ...
    labels_real = Variable(torch.ones(logits_real.shape)).type(dtype)
    labels_fake = Variable(torch.zeros(logits_fake.shape)).type(dtype)
...
def generator_loss(logits_fake, dtype):
    ...
    labels_fake = Variable(torch.ones(logits_fake.shape)).type(dtype)
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
            ...
            if (iter_count % show_every == 0):
                ...
                plt.pause(1)
                plt.savefig('iter_%d.png'%iter_count)
                plt.close()
                print()
