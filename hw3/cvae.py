...
class CVAE(nn.Module):
    def __init__(self, input_size, latent_size, class_size):
        ...
        self.recog = nn.Sequential(
                nn.Linear(input_size + class_size, self.units),
                nn.ReLU(),
                nn.Linear(self.units, self.units),
                nn.ReLU()
            )
        self.recog_mu = nn.Sequential(
                self.recog,
                nn.Linear(self.units, latent_size),
            )
        self.recog_var = nn.Sequential(
                self.recog,
                nn.Linear(self.units, latent_size),
            )
        ...
        self.gen = nn.Sequential(
                nn.Linear(latent_size + class_size, self.units),
                nn.ReLU(),
                nn.Linear(self.units, self.units),
                nn.ReLU(),
    ...
    def recognition_model(self, x, c):
        ...
        xc = torch.cat([x, c], 1)        # (batch_size, input_size + num_classses)
        mu = self.recog_mu(xc)           # (batch_size, units)
        logvar = self.recog_var(xc)      # (batch_size, units)
    ...
    def generation_model(self, z, c): # P(x|z, c)
        ...
        zc = torch.cat([z, c], 1)        # (batch_size, latent_size + num_classses)        
        x_hat = self.gen(zc)             # (batch_size, input_size)
...
def main():
    ...
    v = {'num_workers': 1, 'pin_memory': True} if use_cuda else {}
    train_loader = torch.utils.data.DataLoader(
            datasets.MNIST('./data', train=True, download=True,
                           transform=transforms.ToTensor()),
            batch_size=batch_size, shuffle=True, **v)
